<?php 
	trait ProductModel{
		...
		public function fetchAll($from, $pageSize){
			...
			$conn = Connection::getInstance();
			//thuc thi truy van
			$query = $conn->query("select * from garment where category_id=$id order by id desc limit $from, $pageSize");
		...
		public function totalRecord(){
			...
			$conn = Connection::getInstance();
			//thuc thi truy van
			$query = $conn->query("select id from garment where category_id=$id");
